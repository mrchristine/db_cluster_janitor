...
def cleanup_clusters(url, token, env_name):
  ...
  for c in long_cluster_list:
    ...
    # kill if passthrough cluster / aka single user cluster
      ...
      print("Killing single-user / passthrough cluster: {0}\t User: {1}".format(c['cluster_name'], c['creator_user_name']))
    ...
    # kill long running cluster that should have a shorter auto-termination period
      ...
      print("Clusters should always have auto-terminated enabled. Killing cluster now! {0}\t User: {1}".format(c['cluster_name'], c['creator_user_name']))
      report['autoterminate'].append(c)
    else:
      auto_terminate_hours = c.get('autotermination_minutes', None) / 60 
      if auto_terminate_hours > 12:
        print("Cluster should have auto-termination enabled properly. Killing cluster: {0}\t User: {1}".format(c['cluster_name'], c['creator_user_name']))
        report['autoterminate'].append(c)
      else:
        print("Cluster should have auto-termination enabled. Keeping alive: {0}\t User: {1}".format(c['cluster_name'], c['creator_user_name']))
