...
class JobsClient(dbclient):
  ...
  def find_empty_jobs(self):
    ...
    empty_jobs = filter(lambda x: x['settings'].get('spark_jar_task', None) == x['settings'].get('notebook_task', None), jobs)
    ...
    empty_job_ids = map(lambda x: {'job_id': x['job_id'], 'creator': x['creator_user_name']}, empty_jobs + untilted_jobs)
